...
document.addEventListener("DOMContentLoaded", function() {
  ...
  jQuery('#tish-main-menu > div > ul').unwrap();

        jQuery('#tish-main-menu > ul').attr('id', 'nav-25').addClass('main');


        jQuery('#tish-main-menu > ul li').each(function(){
          if (jQuery(this).find('> ul').length > 0) {
            jQuery(this).find('> a').attr('data-toggle', 'collapse')
              .attr('aria-expanded', 'false')
              .attr('class', 'dropdown-toggle');

              jQuery(this).find('> ul').addClass('collapse list-unstyled sub-menu');
          }
        });
