...
(function (global, factory) {
	...
	// Unwrapping the ul element from div
jQuery('#primary-menu > div > ul').unwrap();

// Adding class to ul
jQuery('#primary-menu > ul').addClass('menu-container');

// Assuming #primary-menu is the correct selector
jQuery('#primary-menu > ul > li').each(function(){
    var $this = jQuery(this);
    
    // If li contains a ul, add classes and additional HTML
    if ($this.find('> ul').length > 0) {
        $this.addClass('menu-item sub-menu');
        $this.find('> a').addClass('menu-link').wrapInner('<div/>');
        $this.find('> ul').addClass('sub-menu-container not-dark').attr('data-class', 'up-lg:not-dark').attr('style', '');
        $this.find('> ul > li').addClass('menu-item').attr('style', '');
        $this.find('> ul > li > a').addClass('menu-link').wrapInner('<div/>');
        
        // Adding sub-menu indicator and trigger button
        $this.find('> a > div').append('<i class="sub-menu-indicator fa-solid fa-caret-down"></i>');
        $this.append('<button class="sub-menu-trigger fa fa-chevron-right"><span class="visually-hidden">Open Sub-Menu</span></button>');
    } else {
        // If li does not contain a ul, add classes and additional HTML
			...
			var _init = function() {
				...
				document.querySelectorAll( '.menu-item' ).forEach(function(el) {
					...
					if( !el.classList.contains('mega-menu-title') && el.querySelectorAll('.sub-menu-container').length > 0 && el.querySelectorAll('.sub-menu-trigger').length < 1 ) {
						...
						subMenuTrigger.classList = 'sub-menu-trigger fa fa-chevron-right';
